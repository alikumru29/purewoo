...
# HTTP bağlantısını oluşturma
conn = http.client.HTTPSConnection(WC_URL)
conn.request("GET", WC_API_ENDPOINT, headers=headers)
res = conn.getresponse()
data = res.read().decode('utf-8')  # Veriyi utf-8 formatında decode ettik

if res.status != 200:
    logging.error(f"API returned {res.status}: {data}")
    exit()
